# Tasks Tracking

Use this doc to track progress job confirmation ke liye.

## Format

For each task, create/update a file like:

- `task-assessment/01-introduction.md`
- `task-assessment/02-frontend-task.md`
- `task-assessment/03-backend-task.md`

Then update this table:

| Task | Title | Status | Notes |
|------|-------|--------|-------|
| 1 | Introduction | Completed |  |
| 2 | Frontend Task | Completed | Dark/Light mode toggle in Navbar, smooth transitions, responsive |
| 3 | Backend Task | Completed | Notes CRUD API (POST/GET/PUT/DELETE), in-memory, all endpoints tested |
| 4 | Blockchain Task | Completed | EVM Smart Contract (PropertyRegistry.sol) + ethers.js API, all endpoints tested |
| 5 | Web3 Task | Completed | MetaMask connect/disconnect, address display, account+network change handlers |
| 6 | AI Task | Completed | Chat API (POST /chat) with intent+entities extraction + mock properties search + frontend Chatbot UI |
| 7 | QA Task | Not started |  |
| 8 | Design Task | Completed | Dashboard /dashboard — Overview, Operations, Insights (loading/empty/alert states) |

